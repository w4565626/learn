 ...
 *
 * 消息可靠性保证：
 * 1. 生产者可靠性保证：发送消息后返回sendResult,如果为true，,则表示消息已经确认发送到服务器并被服务器接收存储。
 * 发送过程是同步的，保证消息送达服务器并返回结果。
 *
 * 2. broker可靠性保证：收到消息，立即写入磁盘，写入成功才返回应答给生产者。
 *
 * 3. 消费者可靠性保证：顺序消费，如果失败，重试5次，仍旧失败，消息存储在消费者本地磁盘，后台线程继续重试。
 * offset存储，默认存储在zoopkeeper上，zoopkeeper通过集群保证数据的安全。并且offset会定期保存，每次重新负载均衡前会强制保存一次。
 */
